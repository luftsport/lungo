...
def _add_to_message(_id, person_id, verify_unique=True):
    # 0. Verify sent already?
    if verify_unique is True:
        where = {'event_id': _id, 'recipient.person_id': person_id}
        verify, _, _, status, _ = get_internal('notifications_messages', **where)
        if status != 200 or len(verify.get('_items', [])) > 0:
            return {"_status": "ERR",
                    "_error": {
                        "code": 422,
                        "message": f'Person {person_id} has already gotten an email - verification is set to True'
                    }}, 422

    # 1. get the notification from _id
    try:
        notification, _, _, status = getitem_internal(resource='notifications', **{'_id': _id})
        if status != 200 or notification.get('status', None) != 'continuous':
            return {"_status": "ERR",
                    "_error": {
                        "code": status,
                        "message": f"Error fetching notification {_id}"
                    }}, status
    ...
    try:
        ...
        if status != 200 or person is None:
            return {"_status": "ERR",
                    "_error": {
                        "code": status,
                        "message": f"Error fetching person {person_id}"
                    }}, status

    except Exception as e:
        app.logger.exception("Error fetching person")

    # 3. now generate a message for this person and insert!

    data = notification.get('data', {})

    payload = {
        'type': notification.get('type', 'email_newsletter'),  # Default type if not provided
        ...
        'sender': notification.get('person_id', 0),  # The sender's user ID
        'event_id': _id,  # Optional event ID if applicable
        'event_created': datetime.utcnow(),
        'event_from': notification.get('event_from', 'smtp_notification'),  # The source of the event
        'event_from_id': notification.get('event_from_id', str(uuid4())),  # Optional ID of the event source
        ...
        'status': 'ready',  # Initial status of the notification
        'unsubscribe': notification.get('unsubscribe', SENDGRID_DEFAULT_UNSUBSCRIBE),  # Unsubscribe link
        'acl': {
            'read': {'users': [person_id], 'roles': []},
            'write': {'users': [], 'roles': []},
            'create': {'users': [], 'roles': []},
            'delete': {'users': [], 'roles': []},
        }
    ...
    if payload['transport'] == 'email':
        app.logger.debug(f"[Notifications cont] Fetching email for recipient {person_id}")
        if notification.get('member_email', 'primary') == 'all':
    ...
    app.logger.debug(f"[Notifications cont] Processing person data for recipient {person_id}")
    if 'date_of_death' in person and person['date_of_death'] is not None:
        app.logger.error(f"[Notifications cont] Person {person_id} is deceased, skipping notification.")
        eve_abort(403, 'Person is deceased')
    if 'subject' in payload['data']:
        app.logger.debug(f"[Notifications cont] Rendering subject for recipient {person_id}")
        subject = subject_template.render(person)
    if 'html_content' in payload['data']:
        app.logger.debug(f"[Notifications cont] Rendering HTML content for recipient {person_id}")
        html_content = html_content_template.render(person)
    if 'plain_text_content' in payload['data']:
        app.logger.debug(f"[Notifications cont] Rendering plain text content for recipient {person_id}")
        plain_text_content = plain_text_content_template.render(person)
    app.logger.debug(f"[Notifications cont] Finished jinja rendering for recipient {person_id}")

    # Prepare the payload for the notification message
    app.logger.debug(f"[Notifications cont] Preparing payload for recipient {person_id}")
    pld = payload.copy()
    ...
    app.logger.debug(f"[Notifications cont] Payload prepared for recipient {person_id}: {pld}")
    ...
    if pld['data'].get('html_content', None) is not None and pld['data'].get('plain_text_content', None) is None:
        ...
        app.logger.debug(f"[Notifications cont] Generating plain text content from HTML for recipient {person_id}")
        soup = BeautifulSoup(pld['data']['html_content'])
        pld['data']['plain_text_content'] = soup.get_text()
        app.logger.debug(f"[Notifications cont] Generated plain text content for recipient {person_id}: {pld['data']['plain_text_content']}")

    try:
        app.logger.debug(f"[Notifications cont] Posting notification message for recipient {person_id}")
        if len(email_addresses) == 0:
            app.logger.error(f"[Notifications cont] No email addresses found for recipient {person_id}, skipping notification generation.")
            return {"_status": "ERR",
                    "_error": {
                        "code": 404,
                        "message": f'Missing email for person {person_id}'
                    }}, 404
        ...
        for email_address in email_addresses:
            if email_address is not None:
                ...
                app.logger.debug(f"[Notifications cont] Notification message posted for recipient {person_id} with {email_address}: {msg_status}")
                return {'_status': 'OK'}, 201  # {},201)
            else:
                app.logger.error(f"[Notifications cont] Invalid email address for recipient {person_id}, skipping this email: {email_address}")

    except Exception as e:
        app.logger.error(f"[Notifications cont cont] Error posting notification message for recipient {person_id}: {e}")

    return {"_status": "ERR",
            "_error": {
                "code": 500,
                "message": "Error posting notification message"
            }}, 500
...
@Notifications.route('/generate/continual/<string:_id>', methods=['POST'])
@require_token()
def add_to_message(_id):
    ...
    elif person_id is not None and int(person_id) > 0:
        message, status_code = _add_to_message(_id, person_id, verify_unique)
        return format_response(message, status_code)

    return format_response({"_status": "ERR",
                            "_error": {
                                "code": 500,
                                "message": "Error posting notification message"
                            }}, 500)
