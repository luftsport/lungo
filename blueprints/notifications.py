...
DEFAULT_FROM_ADDRESS = 'post@nlf.no'
DEFAULT_FROM_NAME = 'Norges Luftsportforbund'
DEFAULT_FROM = {'name': DEFAULT_FROM_NAME, 'email': DEFAULT_FROM_ADDRESS}
# Default reply-to address, human readable name and email address
DEFAULT_REPLY_TO_NAME = 'NLF'
DEFAULT_REPLY_TO_ADDRESS = 'post@nlf.no'
DEFAULT_REPLY_TO = {'name': DEFAULT_REPLY_TO_NAME, 'email': DEFAULT_REPLY_TO_ADDRESS}
# Correct bounce handling
DEFAULT_HEADERS = {'Return-Path': 'bounce@nlf.no'}
...
@Notifications.route('/generate/<string:_id>', methods=['POST'])
@require_token()
    ...
    if status == 200:
        ...
        payload = {
            ...
            'data': {
                ...
                'subject_preamble': data.get('subject_preamble', DEFAULT_PREAMBLE),  # Optional subject preamble
                ...
                'reply_to': data.get('reply_to', DEFAULT_REPLY_TO),  # Default reply-to address
                'from': data.get('from', DEFAULT_FROM),  # Default from address
                ...
                'headers': DEFAULT_HEADERS.update(data.get('headers', {})),  # Optional headers for the email
            ...
            'unsubscribe': response.get('unsubscribe', DEFAULT_UNSUBSCRIBE),  # Unsubscribe link
            'acl': {
...
def email2notification():
    ...
    payload = {
        ...
        'data': {
            ...
            'send_at': data.get('send_at', datetime.utcnow().isoformat()),  # When to send the email (sendgrid)
            'headers': DEFAULT_HEADERS.update(data.get('headers', {})),  # Optional headers for the email
            'unsubscribe': data.get('unsubscribe', DEFAULT_UNSUBSCRIBE)  # Unsubscribe link
        },
        'recipient': None,  # Will be set for each recipient
